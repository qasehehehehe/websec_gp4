    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['change_password'])) {
            ...
            if (!password_verify($current_password, $user['password'])) {
                ...
                $messageType = "error";
            } elseif (strlen($new_password) < 12) {  // CHANGED: From 6 to 12
                $message = "New password must be at least 12 characters long.";
                $messageType = "error";
           } else {
    // ADDED: Password strength validation
    $passwordValidation = validateStrongPassword($new_password);
    if (!$passwordValidation['valid']) {
        $message = $passwordValidation['message'];
        $messageType = "error";
    } else {
        $hashed_password = hashPasswordWithSalt($new_password);  // ← FIXED
            ...
            <div id="password" class="tab-content">
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <small class="text-muted">Password must be at least 12 characters long with uppercase, lowercase, numbers, and special characters.</small>
                    </div>
    ...
    <script>
        ...
        document.getElementById('new_password').addEventListener('input', function() {
            ...
            if (feedback.length > 0) {
                helpText.textContent = 'Missing: ' + feedback.join(', ');
